...
#code now installs required packages 
...
dat$cq_MR[dat$cq_MR > 35] <-NA #MR Cq >35 becomes NA
...
cq_dat$dCt_MR<-(cq_dat$cq_MR - cq_dat$ref_gene) #delta CT MR
...
range(spln$fold_GR, na.rm=TRUE) #range of neg ddCT values for GR 
spln$log_fold_GR<-log((spln$fold_GR))

range(spln$fold_MR, na.rm=TRUE)
hist(spln$fold_MR)
spln$log_fold_MR <-log(spln$fold_MR)
hist(spln$log_fold_MR)
...
cq_dat2$dct_MR<-(cq_dat2$cq_MR - cq_dat2$cq_ACTB_v2) #normalize MR to just ACTB_v2
...
#create a summary statistics dataset for PRLR
...
#ggplot code using summary statistics from above dataset (sprln2_summary_prlr)
bp1<- ggplot(spln2_summary_prlr, aes(x=stage, y=mean_prlr, fill=sex)) + 
        ...
        geom_signif(comparisons=list(c("lay", "hatch")), annotations="p = 0.02",   #geom_signif are significance bars from the model
